...
def calibration_transformation_matrix(width, height, corners):
    ...
    for y in range(0, height):
        for x in range(0, width):
            ...
            p_tx = l * (width-1) * ap_abs/(ap_abs + cp_abs) + m * (width-1) * cp_abs/(ap_abs + cp_abs)
            ...
            p_ty = n * (height-1) * dp_abs/(dp_abs + bp_abs) + o * (height-1) * bp_abs/(dp_abs + bp_abs)
